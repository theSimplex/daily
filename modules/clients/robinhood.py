...
class Robinhood:
    ...
    def login(self,
        ...
        if 'access_token' in data.keys():
            self.headers['access_token'] = data['access_token']
            self.headers['refresh_token'] = data['refresh_token']
            return True
    ...
    def transfers(self):
        ...
        res = self.session.get(endpoints.ach_transfers(), timeout=15)
        res.raise_for_status()  #will throw without auth
        data = res.json()
    ...
    def instrument(self, instrument_id):
        ...
        """
        url = endpoints.instruments(instrument_id)
        try:
    ...
    def watchlists(self, default=True):
        """Wrapper for portfolios
